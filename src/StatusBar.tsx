...
function createStackEntry({
  ...
  return { animated, barStyle, backgroundColor };
}
      ...
      if (lastEntry != null) {
        ...
        if (oldProps?.barStyle !== lastEntry.barStyle) {
          if (Platform.OS === 'android') {
            NativeModules?.EdgeToEdge?.setStatusBarStyle(lastEntry.barStyle);
          } else {
            RNStatusBar.setBarStyle(lastEntry.barStyle, lastEntry.animated);
          }
        }

        if (oldProps?.backgroundColor !== lastEntry.backgroundColor) {
          if (Platform.OS === 'android') {
            NativeModules?.EdgeToEdge?.setNavigationBarBackgroundColor(
              lastEntry.backgroundColor
            );
          }
        }
